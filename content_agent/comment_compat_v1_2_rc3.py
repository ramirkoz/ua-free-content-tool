...
def _telegram_send_media_group(
    ...
    if not 2 <= len(gallery.items) <= 10:
        raise PublishError("Telegram-альбом має містити від 2 до 10 фото.", retryable=False)
...
class CompatibleTelegramPublisher(TelegramBotPublisher):
    ...
    def publish(
        ...
        try:
            ...
            raise PublishError(
                "Telegram: з'єднання перервалося під час публікації альбому. Результат невідомий; автоматичний повтор заблоковано.",
                retryable=False,
                outcome_unknown=True,
            ) from exc
...
class CompatibleLinkedInPublisher(CommentedLinkedInPublisher):
    def publish(self, text: str, progress: dict[str, object], context: PublishContext, media: MediaPayload | ImageGalleryPayload | None = None) -> PublishResult:
        # v1.2 final policy: LinkedIn keeps the donation block in the root post.
