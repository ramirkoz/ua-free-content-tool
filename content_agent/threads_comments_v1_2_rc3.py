...
class CommentedThreadsPublisher(SafeThreadsPublisher):
    ...
    def _publish_text_reply_once(
        ...
        *,
        ...
        try:
            ...
            # If Meta definitively rejected the request, it is safe to clear the
            # pre-write marker. Transport/5xx ambiguity still stays fail-closed.
    ...
    def _publish_gallery_root(
        ...
        if progress.get("threads_gallery_publish_started") and not progress.get("threads_gallery_remote_id"):
            try:
                ...
                status = ""
            if status == "PUBLISHED":
                progress = {**progress, "threads_gallery_remote_id": container_id}
                context.save_progress(progress)
                return PublishResult(remote_id=container_id, progress=progress)
    ...
    def _finish_legacy_donation_if_possible(
        ...
        if bool(progress.get(_PUBLISH_STARTED)):
            status = self._container_status(container_id)
            if status == "PUBLISHED":
                progress = finish_phase(
                    progress,
    ...
    def publish(
        ...
        # v1.2.0 sometimes got the publish response lost after Meta had already
        # published the donation reply. Reconcile that durable legacy state
        # before considering a new write.
