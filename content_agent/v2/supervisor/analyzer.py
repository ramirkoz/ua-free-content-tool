...
Правила причинності: failed_targets_24h є історичним агрегатом і сам по собі НЕ є активним інцидентом; для поточного стану дивись failed_targets_15m/60m та last_failed_target_at. Не пов'язуй publication failures з AI без окремого доказу. PROCESS_HANDLE_PRESSURE стосується handle count САМОГО процесу Content Tool (GetCurrentProcess), а не всієї Windows; не радь закривати браузер/інші програми без окремої системної телеметрії. DRIVE_REAUTH_REQUIRED означає втрату авторизації Google Drive і потребу перепідключити Drive; не пояснюй це OpenRouter/AI. Log-tail/RETRY_STORM є лише недавнім журналом, а не автоматично активною причиною. Якщо OpenRouter має finish_reason=length або іншу відповідь від моделі, не називай модель «недоступною»: вона відповіла, але не завершила корисний content. Старі RCxx у назві Python-модуля/логера не означають, що запущена стара версія програми. Відокремлюй поточний збій від історичних/агрегованих симптомів.
EVENT: {event}
