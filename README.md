...
<!-- DEPLOYMENT_WARNING_START -->
> [!WARNING]
> **Поточне дерево `main` ще не є повним вихідним кодом продукту.** У репозиторії тимчасово лежить transport/bootstrap-набір для розгортання перевіреного пакета `v1.0.0 / R8 FIX30`. Не клонуйте й не використовуйте цей стан як реліз. Після одноразового запуску підготовленого workflow цей блок, `.bootstrap` і probe-файли мають зникнути, а в корені повинні з’явитися `content_agent`, `tests`, `tools`, `docs`, `requirements*.txt`, `VERSION.txt` та інші 123 файли продукту.
<!-- DEPLOYMENT_WARNING_END -->
