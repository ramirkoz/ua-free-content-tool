...
def main() -> None:
    telegram = compose_publication_text_rc3(
        "News",
        "telegram",
    ...
    for platform in ("facebook", "threads", "linkedin", "instagram"):
        root = compose_publication_text_rc3(
            "News",
            platform,
            include_source_link=False,
            source_url="",
    ...
    decoded = decode_rewrite_payload_rc3(
    ...
    group = SimpleNamespace(
        ...
        combined_text="Metadata wrapper that should not control short-mode length",
        include_source_link=False,
        source_count=1,
    ...
    config = AppConfig()
    check(not config.platform_ready("instagram"), "Instagram must remain disabled until credentials are configured")
